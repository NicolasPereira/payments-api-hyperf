...
final class TransferAtomicTest extends TestCase
{
    ...
    public function testLockForUpdateOrderedAscUserId(): void
    {
        ...
        try {
            $ordered = Database::lockWalletsForUpdate([$payeeId, $payerId]);
            // Should be ordered asc by user_id
            $ids = [];
            foreach ($ordered as $row) {
                $ids[] = (int) $row->user_id;
            }
            sort($ids);
            // The returned collection order should be asc
            $first = (int) $ordered->first()->user_id;
            $last = (int) $ordered->last()->user_id;
            self::assertLessThanOrEqual($last, $first + max($payerId, $payeeId));
            self::assertSame(min($payerId, $payeeId), $first);
            self::assertSame(max($payerId, $payeeId), $last);
            ...
            Database::transaction(function () use ($payerId, $payeeId) {
                ...
                self::assertSame(min($payerId, $payeeId), (int) $locked->first()->user_id);
            });
        } finally {
    ...
    public function testRecheckBalanceAfterLockPreventsNegative(): void
    {
        ...
        } catch (\App\Domain\Wallet\Exception\InsufficientBalanceException $e) {
    ...
    public function testConcurrentTwoTransfersOnlyOneSucceedsNoNegative(): void
    {
        ...
        $redisMock->method('storeResult')->willReturn(null);
        ...
        try {
            ...
            self::assertSame(422, $e->getHttpStatus());
        } catch (\App\Domain\Transfer\Exception\TransferValidationException $e) {
        ...
        $redisMock2->method('storeResult')->willReturn(null);
